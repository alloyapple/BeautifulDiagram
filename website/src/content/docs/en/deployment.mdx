...
BeautifulDiagram production environment consists of the following services:

| Service | Tech Stack | Image |
|---------|-----------|-------|
| **Frontend** | React 19 + Nginx | `alloyapple/beautifuldiagram-frontend:latest` |
| **Backend** | FastAPI + LangGraph | `alloyapple/beautifuldiagram-backend:latest` |
...
```bash
mkdir beautifuldiagram && cd beautifuldiagram
```
...
services:
  db:
    ...
    container_name: beautifuldiagram-db
    restart: always
    environment:
      POSTGRES_USER: postgres
      POSTGRES_PASSWORD: your_secure_password
      POSTGRES_DB: beautifuldiagram
    ports:
      - "5432:5432"
    volumes:
  ...
  backend:
    image: alloyapple/beautifuldiagram-backend:latest
    container_name: beautifuldiagram-backend
    restart: always
    depends_on:
      - db
    environment:
      - DATABASE_URL=postgresql+asyncpg://postgres:your_secure_password@db:5432/beautifuldiagram
  ...
  frontend:
    image: alloyapple/beautifuldiagram-frontend:latest
    container_name: beautifuldiagram-frontend
    restart: always
    depends_on:
      - backend
    ports:
...
```bash
docker exec beautifuldiagram-db pg_dump -U postgres beautifuldiagram > backup.sql
```

### Restore Database

```bash
cat backup.sql | docker exec -i beautifuldiagram-db psql -U postgres beautifuldiagram
```
...
```bash
git clone https://github.com/alloyapple/BeautifulDiagram.git
cd BeautifulDiagram
